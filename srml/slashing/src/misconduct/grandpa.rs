...
/// Reject set of votes
///
/// Collusion that entails rejecting a set of the same type of votes (pre-commits or pre- votes)
/// In a round `r` contradicting such that it is impossible to have supermajority for a block `B`
// TODO(niklasad1): impl is missing
pub struct RejectSetVotes<W, S> {
	votes: rstd::vec::Vec<Vote<W, S>>,
}
